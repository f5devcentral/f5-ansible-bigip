...
from ..module_utils.common import (
    F5ModuleError, AnsibleF5Parameters, ImishConfig
)
...
class ModuleManager(object):
    ...
    def load_config_on_device(self, name):
        ...
        if 'commandResult' in response['contents']:
            if 'Dynamic routing is not enabled' in response['contents']['commandResult']:
    ...
    def read_current_from_device(self):
        ...
        if 'commandResult' in response['contents']:
            if 'Dynamic routing is not enabled' in response['contents']['commandResult']:
                raise F5ModuleError(response['contents']['commandResult'])
        return response['contents']['commandResult']
    ...
    def execute_show_commands(self, commands):
        ...
        for command in to_list(commands):
            ...
            if 'commandResult' in response['contents']:
                if 'Dynamic routing is not enabled' in response['contents']['commandResult']:
                    raise F5ModuleError(response['contents']['commandResult'])
                body.append(response['contents']['commandResult'])
